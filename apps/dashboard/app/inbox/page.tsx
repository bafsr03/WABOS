...
import { Bot, Send, MessageCircle, UserRound, Sparkles, ChevronLeft } from 'lucide-react';
...
interface Conversation {
  id: number; mode: 'ai' | 'human'; unread_count: number; last_message_at: number;
...
export default function InboxPage() {
  ...
  const loadMessages = useCallback((id: number) => {
    api<{ messages: Message[] }>(`/api/conversations/${id}/messages`).then((r) => setMessages(r.messages)).catch(() => {});
  ...
  async function send(e: React.FormEvent) {
    ...
    setSending(true);
    try {
      await api(`/api/conversations/${selectedId}/messages`, { method: 'POST', body: JSON.stringify({ text: draft.trim() }) });
      setDraft('');
            ...
            <>
              <div className="glass flex items-center justify-between border-b border-border px-4 py-3 sm:px-5">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-fg">{selected.name || `+${selected.phone}`}</div>
                    <div className="tabular text-xs text-subtle">+{selected.phone}</div>
                  </div>
                </div>
              ...
              <form onSubmit={send} className="glass flex items-center gap-2 border-t border-border p-3">
                <input value={draft} onChange={(e) => setDraft(e.target.value)}
                  placeholder="Escribe una respuesta… (pasa la conversación a modo humano)"
