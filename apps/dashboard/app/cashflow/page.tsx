...
interface Position { cashSales: number; income: number; expenses: number; net: number }
...
export default function CashflowPage() {
  ...
  const load = useCallback(() => {
    api<Movement[]>('/api/cash-movements').then(setMovements).catch(() => {});
    api<Position>('/api/cash/position').then(setPosition).catch(() => {});
  ...
  return (
    <Shell>
      <div className="mx-auto max-w-3xl p-6 lg:p-8">
        <PageHeader title="Caja" subtitle="Movimientos del día: ventas, gastos e ingresos extra." />

        {/* Caja — Hoy */}
        <Card className="mb-6 p-5">
          <p className="mb-4 flex items-center gap-2 text-sm font-semibold text-fg"><Coins size={16} /> Caja — Hoy</p>
        ...
        <div className="mt-6">
          <h2 className="mb-3 text-sm font-semibold text-fg">Movimientos de hoy</h2>
          {movements.length === 0 ? (
